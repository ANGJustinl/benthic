...
export const STORY_EVENTS = {
  // Chapter 1: Stage I - The Cold Boot
  ...
  // Chapter 1: Stage II - The Rust Lung
  ...
  // Chapter 1: Stage III - The Ghost in the Machine
...
export function createStorySequence(sequenceName: keyof typeof STORY_SEQUENCES, generateLogId: () => string): any[] {
  ...
  const result = sequence.map((eventKey, index) => ({
    id: generateLogId(),
    text: STORY_EVENTS[eventKey as keyof typeof STORY_EVENTS],
    type: 'info' as const,
    timestamp: baseTime - ((sequence.length - 1 - index) * 100), // 第一个事件时间戳最旧，显示在最下面
    isStorySequence: true, // 标记为故事序列
    sequenceName: sequenceName,
    sequenceIndex: index
  }));
  
  console.log(`Created story sequence ${sequenceName}:`, result.map(r => ({ text: r.text, timestamp: r.timestamp, isStorySequence: r.isStorySequence })));
